...
public class NinePuzzle{
	...
	// Based off of pseudocode that was given
	public static Graph BuildNPuzzleGraph(int N) {
		
		Graph graph = new Graph(NUM_BOARDS);
		
		for (int current_vertex = 0; current_vertex < graph.V(); current_vertex++) {
			int B[][] = getBoardFromIndex(current_vertex);
			int n = (int)Math.sqrt(N);
			int i = 0;
			int j = 0;
			boolean done = false;
			
			// Find where the free space is in the board
			// i, j will result in its coordinates
			for (i=0; i < n; i++) {
				for (j=0; j < n; j++)  {
					if (B[i][j] == 0) {
						done = true;
			...
			// Code to move the piece modelled from given pseudocode
			if (i > 0) {
				int Bprime[][] = new int[n][n];
				for (int a = 0; a < n; a++) {
					for (int b = 0; b < n; b++) {
						Bprime[a][b] = B[a][b];
					}
				}
				int temp = Bprime[i-1][j];
				Bprime[i-1][j] = 0;
				Bprime[i][j] = temp;
				int index = getIndexFromBoard(Bprime);
				graph.addEdge(index, current_vertex);
			}
	...
	*/
	public static boolean SolveNinePuzzle(int[][] B){
		// We hardcode in 9 but our BuildNPuzzleGraph method needs
		// to be based off a value N?
		
		// Input board vertex is getIndexFromBoard(B)
		
		int vertex = getIndexFromBoard(B);
		
		// The goal state, u, would be 0
		Graph graph = BuildNPuzzleGraph(9);
		
		// Run BFS on the graph starting at u
		BreadthFirstPaths path = new BreadthFirstPaths(graph, 0);
		
		// If a path exists, print all boards in the path
		if (path.hasPathTo(vertex)) {
			for (int i : path.pathTo(vertex)) {
				printBoard(getBoardFromIndex(i));
			}
		}
		return path.hasPathTo(vertex);
	}
...
/***************************************************************************************
BreadthFirstPaths is taken from Algorithms by Sedgewick & Wayne (page 540)
*/
...
/********************************************************************************************
Graph is taken from Algorithms by Sedgewick and Wayne (page 526)
*/
class Graph {
...
/********************************************************************************************
Bag taken from the textbook website
*/
